...
/// <summary>
/// Sets explicit chart visibility for an entity in a model-driven app.
/// Requires either --all (include all charts) or one or more --chart options.
/// </summary>
public static class SetChartsCommand
{
    public static Command Create()
    {
        ...
        {
            Description = "Include all charts (removes explicit selections, enabling include-all mode)"
        };

        var chartOption = new Option<string[]>("--chart")
        {
            Description = "Chart name to include explicitly (can be specified multiple times). Requires --all or at least one --chart.",
            Arity = ArgumentArity.ZeroOrMore,
            AllowMultipleArgumentsPerToken = false
        };

        var command = new Command("set-charts", "Set explicit chart visibility for a table in the app. Requires --all or at least one --chart.")
        {
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                var msg = all
                    ? $"Set charts to include-all for '{entity}' in app '{appName}'."
                    : $"Set {chartNames.Length} explicit chart(s) for '{entity}' in app '{appName}'.";
                Console.Error.WriteLine(msg);
            }
            else
            {
                writer.WriteSuccess(new { success = true, app = appName, entity, all, charts = chartNames });
            }
        ...
        {
            var error = ExceptionMapper.Map(ex, context: $"setting charts for entity '{entity}' in app '{appName}'", debug: globalOptions.Debug);
