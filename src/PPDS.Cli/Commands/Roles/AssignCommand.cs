...
public static class AssignCommand
{
    public static Command Create()
    {
        ...
        var command = new Command("assign", "Assign a role to a user")
        {
            roleArgument,
            userOption,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!globalOptions.IsJsonMode)
            {
                Console.Error.WriteLine($"Assigning role '{roleInfo.Name}' to user '{userInfo.FullName ?? userInfo.DomainName}'...");
            }

            await roleService.AssignRoleAsync(userInfo.Id, roleInfo.Id, cancellationToken);

            if (globalOptions.IsJsonMode)
            {
                var output = new AssignOutput
                {
                    UserId = userInfo.Id,
            ...
            {
                Console.Error.WriteLine("Role assigned successfully.");
            }
        ...
        {
            var error = ExceptionMapper.Map(ex, context: $"assigning role '{role}' to user '{user}'", debug: globalOptions.Debug);
            writer.WriteError(error);
    ...
    private sealed class AssignOutput
    {
