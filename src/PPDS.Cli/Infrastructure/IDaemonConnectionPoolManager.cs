...
/// </summary>
public interface IDaemonConnectionPoolManager : IAsyncDisposable
{
    /// <summary>
    ...
    /// <summary>
    /// Gets or creates a cached service provider for the given profiles and environment.
    /// The returned provider contains all Dataverse services (IQueryExecutor, ICachedMetadataProvider,
    /// ISolutionService, IDataverseConnectionPool, etc.) and is long-lived — do NOT dispose it.
    /// </summary>
    /// <param name="profileNames">List of profile names (for multi-profile pooling). Order does not matter.</param>
    /// <param name="environmentUrl">Environment URL (will be normalized).</param>
    /// <param name="deviceCodeCallback">Optional callback for device code flow.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>A cached service provider. Caller must NOT dispose it.</returns>
    Task<IServiceProvider> GetOrCreateServiceProviderAsync(
        IReadOnlyList<string> profileNames,
        string environmentUrl,
        Action<DeviceCodeInfo>? deviceCodeCallback = null,
        CancellationToken cancellationToken = default);
