using System;

namespace PPDS.Dataverse.Metadata.Authoring;

/// <summary>
/// Result of a create global choice operation.
/// </summary>
public sealed class CreateChoiceResult
{
    /// <summary>Gets or sets the name of the created global choice.</summary>
    public string Name { get; set; } = "";

    /// <summary>Gets or sets the metadata identifier of the created global choice.</summary>
    public Guid MetadataId { get; set; }

    /// <summary>Gets or sets whether the operation was a dry run.</summary>
    public bool WasDryRun { get; set; }

    /// <summary>Gets or sets the validation messages produced during the operation.</summary>
    public ValidationMessage[] ValidationMessages { get; set; } = [];
}
