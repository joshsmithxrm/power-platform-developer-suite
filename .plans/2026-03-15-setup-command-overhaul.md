# Setup Command Overhaul Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `/setup` to cover prerequisites, dependencies, builds, tool installation, AI tooling, and a non-interactive `--update` mode — taking a fresh machine to productive in one command.

**Architecture:** Single-file rewrite of `.claude/commands/setup.md`. The command is a markdown instruction file that Claude Code interprets — not executable code. No tests. The structure changes from a 5-step wizard to a 12-step wizard with a separate `--update` flow.

**Tech Stack:** Markdown (Claude Code command format)

**Spec:** `specs/setup-command.md`
...
```
.claude/commands/setup.md    ← Rewrite (223 lines → ~350 lines)
```

Single file. No new files, no splits.

---

## Chunk 1: Rewrite setup.md

### Task 1: Rewrite the Setup Command

**Files:**
- Modify: `.claude/commands/setup.md` (full rewrite)

- [ ] **Step 1: Read the existing command and spec**

Read both files to understand current structure and target:
- `.claude/commands/setup.md` (current)
- `specs/setup-command.md` (target spec)

- [ ] **Step 2: Write the new setup.md**

Replace the entire contents of `.claude/commands/setup.md` with the following:
...
Read `.claude/commands/setup.md` and verify:
- Step 0 (platform detection) exists
- Step 1 (prerequisites) has all 5 tools with version checks
- Step 3 (repos) includes vault
- Step 5 (dependencies) includes dotnet restore, npm install for extension and tui
- Step 6 (build) includes dotnet build and ext:compile
- Step 7 (tools) includes CLI and VSIX (Windows only)
- Step 8 (AI tooling) checks superpowers path
- Step 9 (DX) has sound using `pwsh` not `powershell`
- Update mode section exists with non-interactive flow
- Repository URLs table includes vault

- [ ] **Step 4: Commit**

```bash
git add .claude/commands/setup.md
git commit -m "feat(tools): rewrite /setup with prerequisites, deps, builds, --update mode

Add prerequisites check (git, node, dotnet, pwsh, code), dependency
installation (dotnet restore, npm install), build verification,
tool installation (CLI + VSIX), AI tooling check (superpowers),
platform detection (Windows + Linux), vault repo, and non-interactive
--update mode for daily refresh."
```
...
After the rewrite, manually verify by reading the command file:
- All spec requirements from `specs/setup-command.md` are covered
- No `powershell` references (must be `pwsh`)
- No shell redirections in instructional text (except inside platform-specific scripts that the user runs directly)
- vault has "Private — ask maintainer" note
- `--update` mode has no AskUserQuestion calls
- Platform detection gates Windows-only steps
