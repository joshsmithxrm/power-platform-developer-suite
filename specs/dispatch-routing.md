...
4. **Interactive completion detection.** `BgHandle.poll()` reads `~/.claude/jobs/<short>/state.json` and returns one of `"working"`, `"done"`, `"blocked"`, `"error"` based on `state["state"]`. `"blocked"` includes the `state["needs"]` text in the `BlockedSessionError` raised by `wait()`. **Defense-in-depth (#1138):** `poll()` also returns `"blocked"` when `state["tempo"] == "blocked"` AND `state["block"]["questions"]` is a non-empty list, regardless of the top-level `state["state"]` field — the daemon-internal asymmetry that an accidental `AskUserQuestion` call produces from a bg/headless session before the ~15-60s auto-answer fires. Workers must use `Skill(skill='await-operator', ...)` (#1137) for real ratification gates; this is the supervisor's catch for the contract violation.
