...
- All `--bg` invocations from unattended daemons (`pipeline.py`, `pr_monitor.py`) pass `--dangerously-skip-permissions`. Foreground `start-bg-spawn.py` does NOT pass it (a human is attached).
...
| AC-17 | All `pipeline.py` and `pr_monitor.py` `--bg` invocations include `--dangerously-skip-permissions`; `start-bg-spawn.py` does NOT | `test_dangerous_flag_unattended_only` | 🔲 |
