...
| AC-40 | Runtime plug-in discovery follows resolvable external base classes (including constructed generic bases) and derived interfaces, keeps unrelated unresolved interface lineages isolated, excludes open generic implementations, and returns actionable `--reference-dir` guidance when an unresolved lineage prevents finding any candidate | `Extract_RuntimePluginInheritedThroughExternalBase_DetectsConcreteType`, `Inspect_RuntimePluginInheritedThroughExternalGenericBase_DetectsConcreteType`, `Extract_RuntimePluginThroughExternalDerivedInterface_DetectsConcreteType`, `Inspect_RuntimePluginWithUnresolvedUnrelatedSdkInterface_KeepsPluginCandidate`, `Extract_OpenGenericRuntimePlugin_RejectsPackageWithoutDeployableCandidate`, `Extract_UnresolvedExternalBase_ExplainsReferenceDirRecovery`, `Extract_UnresolvedExternalGenericBase_ExplainsReferenceDirRecovery` | ✅ |
...
| Unrelated unresolved interface | A valid plug-in assembly also exports a helper implementing an unavailable non-`IPlugin` interface | Proven runtime plug-ins remain candidates; the helper lineage does not abort assembly inspection |
| Open generic plug-in | Public non-abstract `Plugin<T> : IPlugin` | Excluded from runtime candidates because Dataverse cannot instantiate the open type |
