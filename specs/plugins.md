...
- A runtime `IPlugin` assembly with no PPDS registration attributes uses its manifest simple name and records its concrete runtime type names in `allTypeNames`; extraction does not invent steps or annotated type registrations. Detection follows resolvable base classes and derived interfaces across assembly boundaries, verifies the complete referenced assembly identity (including `Microsoft.Xrm.Sdk` version 9.0.0.0, neutral culture, and its official public-key token) rather than trusting simple names, and reports unresolved inheritance with `--reference-dir` guidance. `PluginStep` and `CustomApi` evidence is trusted only from the official strong-named `PPDS.Plugins` assembly, and every annotated handler must also be a public, concrete, closed, instantiable runtime `IPlugin`.
...
| AC-48 | `PluginStep`/`CustomApi` evidence is bound to the official strong-named `PPDS.Plugins` identity (name, neutral culture, public-key token; version unpinned); local, wrong-token, and forwarded lookalikes do not qualify, and every official annotated handler is validated before deployability filtering as a public, concrete, closed runtime `IPlugin` | `Extract_LookalikeRegistrationAttribute_DoesNotQualifyAssembly`, `Extract_SameNameWrongTokenPpdsAttribute_DoesNotQualifyAssembly`, `DeployAssemblyAsync_SpoofedRegistrationMetadata_FailsBeforeServerCalls`, `Extract_OfficialRegistrationOnNonPluginHandler_RejectsPackage`, `Extract_MixedValidPluginAndInvalidOfficialHandler_RejectsPackage`, `DeployAssemblyAsync_OfficialRegistrationOnNonPluginHandler_FailsBeforeServerCalls`, `DeployAssemblyAsync_MixedInvalidOfficialHandler_FailsBeforeServerCallsEvenWithMatchingDigest` | ✅ |
...
| AC-51 | Deploy ties evidence to each configured runtime, step-owner, and Custom API handler TypeDef and verifies it still exists and remains public, concrete, closed, and an exact runtime `IPlugin`; removing, renaming, privatizing, abstracting, opening, or stripping one type fails before dry-run lookup or real upload even when another valid plug-in remains | `DeployAssemblyAsync_RebuiltPackageRemovesConfiguredType_FailsBeforeLookupOrUpload`, `DeployAssemblyAsync_ConfiguredTypeLosesRuntimeDeployability_FailsBeforeServerCalls` | ✅ |
