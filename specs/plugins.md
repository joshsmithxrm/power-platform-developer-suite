...
- A runtime `IPlugin` assembly with no PPDS registration attributes uses its manifest simple name and records its concrete runtime type names in `allTypeNames`; extraction does not invent steps or annotated type registrations. Detection follows resolvable base classes and derived interfaces across assembly boundaries, verifies the complete referenced assembly identity (including the strong-named `Microsoft.Xrm.Sdk` identity) rather than trusting simple names, and reports unresolved inheritance with `--reference-dir` guidance. `PluginStep` and `CustomApi` evidence is trusted only from the official strong-named `PPDS.Plugins` assembly, and every annotated handler must also be a concrete runtime `IPlugin`.
- Before authentication, environment resolution, or any Dataverse call, dry-run and real deploy canonicalize and buffer every configured artifact exactly once. Repeated paths, duplicate root package IDs, a stale configured identity, any missing configured plug-in type, stripped plug-in evidence, or any additional plausible primary abort the whole configuration. The retained bytes drive inspection and upload, so concurrent path replacement cannot create a validate/upload split or partial application of later assemblies and Custom APIs.
- Extraction records a package-content SHA-256 and performs metadata, containment, and extraction against one immutable snapshot. Deploy normally proves the configured owner and package-wide ambiguity from package-local metadata; the digest preserves an unchanged `--reference-dir` extraction without persisting machine-specific resolver paths, while any changed package whose external ancestry cannot be reproven fails closed and must be re-extracted.
...
| AC-42 | Runtime plug-in matching requires a direct reference to the strong-named `Microsoft.Xrm.Sdk` assembly identity; local type definitions, wrong-name or wrong-token lookalikes, and type-forwarded non-SDK identities fail closed | `Extract_ZeroAttributeRuntimePlugin_UsesManifestNameAndDoesNotInventSteps`, `Extract_SpoofedPluginInterfaceFromUnrelatedAssembly_RejectsPackage`, `Extract_LocalTypeDefinitionNamedIPlugin_RejectsPackage`, `Extract_IPluginFromMicrosoftXrmSdkWithWrongToken_RejectsPackage`, `Extract_IPluginForwardedFromNonSdkIdentity_RejectsPackage` | ✅ |
...
| AC-47 | Buffered package-wide ambiguity detection follows package-local `TypeReference` and constructed-generic `TypeSpecification` inheritance with cycle protection; full external AssemblyRef identity must match, and unresolved/malformed added candidates fail closed before dry-run lookup or upload | `DeployAssemblyAsync_RebuiltPackageAddsInheritedPlugin_FailsBeforeServerCalls`, `DeployAssemblyAsync_AddedInheritanceWithWrongTokenDependency_FailsClosedBeforeServerCalls`, `Extract_WrongTokenSameNameInheritanceDependency_DoesNotProvePlugin` | ✅ |
| AC-48 | `PluginStep`/`CustomApi` evidence is bound to the official strong-named `PPDS.Plugins` identity (name, neutral culture, public-key token; version unpinned); local, wrong-token, and forwarded lookalikes do not qualify, and official annotations on non-`IPlugin` handlers fail validation before any server call | `Extract_LookalikeRegistrationAttribute_DoesNotQualifyAssembly`, `Extract_SameNameWrongTokenPpdsAttribute_DoesNotQualifyAssembly`, `DeployAssemblyAsync_SpoofedRegistrationMetadata_FailsBeforeServerCalls`, `Extract_OfficialRegistrationOnNonPluginHandler_RejectsPackage`, `DeployAssemblyAsync_OfficialRegistrationOnNonPluginHandler_FailsBeforeServerCalls` | ✅ |
| AC-49 | Extraction validates containment and reads metadata from one immutable package snapshot and records its SHA-256; deploy rejects changed content with stripped evidence while accepting an unchanged package whose ancestry was proven with `--reference-dir` | `Inspect_BufferedSnapshotDoesNotReopenReplacedPackagePath`, `DeployAssemblyAsync_StrippedPluginEvidence_FailsBeforeServerCalls`, `DeployAssemblyAsync_ConfigExtractedWithReferenceDir_DryRunDoesNotReloadDependencyGraph` | ✅ |
...
| AC-51 | Deploy verifies that every configured runtime, step-owner, and Custom API handler type still exists in the selected buffered package assembly; removing or renaming one type fails before dry-run lookup or real upload | `DeployAssemblyAsync_RebuiltPackageRemovesConfiguredType_FailsBeforeLookupOrUpload` | ✅ |
