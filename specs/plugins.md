...
- Before authentication, environment resolution, or any Dataverse call, dry-run and real deploy canonicalize and buffer every configured artifact exactly once. Repeated paths, duplicate root package IDs, a stale configured identity, stripped plug-in evidence, or any additional plausible primary abort the whole configuration. The retained bytes drive inspection and upload, so concurrent path replacement cannot create a validate/upload split or partial application of later assemblies and Custom APIs.
