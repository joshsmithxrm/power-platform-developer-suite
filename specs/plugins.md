...
| AC-47 | Buffered package-wide ambiguity detection follows package-local `TypeReference` and constructed-generic `TypeSpecification` inheritance with cycle protection; full external AssemblyRef identity must match, and unresolved/malformed added candidates fail closed during extraction and before dry-run lookup or upload | `DeployAssemblyAsync_RebuiltPackageAddsInheritedPlugin_FailsBeforeServerCalls`, `DeployAssemblyAsync_AddedInheritanceWithWrongTokenDependency_FailsClosedBeforeServerCalls`, `DeployAssemblyAsync_AddedUnresolvedCandidateAncestry_FailsBeforeServerCalls`, `Extract_WrongTokenSameNameInheritanceDependency_DoesNotProvePlugin`, `Extract_MixedPackageWithUnresolvedCandidateAncestry_FailsClosed` | ✅ |
...
| AC-50 | Cancellation propagates immediately from local preflight or deployment and cannot be converted into a failed result followed by profile creation or later mutation | `ExecuteAsync_CancellationDuringPreflight_DoesNotCreateProfileOrContinue`, `DeployAssemblyAsync_CancellationDuringUpload_RethrowsAndStopsDeployment` | ✅ |
