...
| AC-04 | When the banner is unparseable, the identify-session step scans `~/.claude/jobs/*/state.json` and selects the entry whose `cwd` matches the requested worktree path and whose `createdAt` is within the last 10 seconds | `test_start_bg_spawn.py::test_fallback_id_lookup` (uses tmp_path with fixture state.json files at varying createdAt offsets) | ❌ |
