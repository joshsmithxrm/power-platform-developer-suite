...
6. **Install dependencies** (if ppds selected): `dotnet restore PPDS.sln`, `npm install --prefix src/extension`, `npm install --prefix tests/tui-e2e`
7. **Build verification** (if ppds selected): `dotnet build PPDS.sln -v q`, `npm run ext:compile`. Stop if either fails.
8. **Install tools** (if ppds selected): Run `pwsh scripts/Install-LocalCli.ps1` for CLI. On Windows, run `npm run ext:local --prefix src/extension` for extension VSIX.
...
4. **Install dependencies**: `dotnet restore PPDS.sln`, `npm install --prefix src/extension`, `npm install --prefix tests/tui-e2e`
