...
| AC-194 | **(v9.1 — Notification)** Terminal-state enum is exactly: `ready`, `stuck-ci-fix-exhausted`, `stuck-triage-exhausted`, `stuck-thrash-detected`, `ci-timeout`, `monitor-crash` — written to `.workflow/pr-monitor-result.json` `status` field and to the notification body | `test_pr_monitor.py::test_terminal_state_enum` | 🔲 |
