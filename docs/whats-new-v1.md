...
- **Metadata authoring** — `ppds metadata entity|attribute|relationship|optionset|key create|update|delete` with validation, dry-run, and solution awareness. Status reason management via `entity add-statusreason|list-statusreasons|update-statusreason|remove-statusreason`. `ppds publish entity <name>...` for entity-type publishing.
