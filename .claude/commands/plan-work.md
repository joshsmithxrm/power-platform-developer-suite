...
## Issues
<For each issue: number, title, 2-3 sentence summary, creation date>
...
Before implementing, plan mode MUST verify:

1. **Current patterns** - Explore how similar functionality is implemented today
2. **ADR compliance** - Check docs/adr/ for relevant architecture decisions
3. **Codebase changes** - Issues created on <date>. Has the affected code changed?
4. **Approach validation** - Does the issue's approach still make sense?

⚠️ If verification reveals the issue's approach is outdated or conflicts with
current patterns/ADRs, flag this before implementing.

## Research Hints

<Auto-generated based on issue content - analyze titles, descriptions, and labels>
- Explore existing <related feature> patterns in src/<relevant-area>
- Check how <similar functionality> is implemented
- Review ADR-<N> for <relevant-topic>
- Look at <related-file>.cs for reference implementation
...
3. Explore code using research hints
4. Create implementation plan
5. Get user approval before executing
```

**Generating Research Hints:**

Analyze issue content to generate relevant hints:
- Issue mentions "authentication" → "Explore src/PPDS.Auth/"
- Issue mentions "CLI" → "Check existing commands in src/PPDS.Cli/Commands/"
- Issue references another issue → "Review #X for context"
- Issue has "breaking-change" label → "Check CHANGELOG.md patterns"
- Issue mentions "bulk" → "Review BulkOperationExecutor.cs patterns"
...
- **/create-worktree**: Create worktree for ad-hoc work (no GitHub issue required)
