...
- **Visual verification (MANDATORY for UI changes):** After any CSS, layout, HTML template, or message wiring change, use `/verify extension` to take a screenshot and verify rendering. A passing typecheck is not proof of correct rendering. See `/verify` `REFERENCE.md §ext` for the verification protocol. <!-- enforcement: T3 -->
