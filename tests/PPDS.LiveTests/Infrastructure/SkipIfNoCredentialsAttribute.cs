...
/// <summary>
/// Fact attribute for destructive CLI E2E tests that actually modify Dataverse state.
/// These tests are excluded from default CI runs via the DestructiveE2E trait.
/// Use with [Trait("Category", "DestructiveE2E")] on each test method.
/// </summary>
/// <remarks>
/// Destructive tests:
/// - Actually deploy plugins to Dataverse
/// - Actually clean up orphaned registrations
/// - Must clean up after themselves in test teardown
/// - Require dedicated test environment
/// - Excluded from default CI: --filter "Category!=DestructiveE2E"
/// </remarks>
public sealed class DestructiveE2EWithCredentialsAttribute : FactAttribute
{
    private static readonly LiveTestConfiguration Configuration = new();
    ...
    /// Initializes a new instance that skips on non-.NET 8.0 or missing credentials.
    /// </summary>
    public DestructiveE2EWithCredentialsAttribute()
    {
        if (!IsNet8Runtime())
        {
            Skip = "CLI E2E tests only run on .NET 8.0 (CLI is spawned with --framework net8.0, making other TFMs redundant).";
