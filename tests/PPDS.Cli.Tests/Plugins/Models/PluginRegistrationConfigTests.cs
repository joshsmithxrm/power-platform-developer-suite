...
public class PluginRegistrationConfigTests
{
    ...
    [Fact]
    public void Validate_DuplicateExplicitStepNames_ThrowsWithActionableMessage()
    {
        // Both steps have the same explicit Name on the same plugin type.
        var config = new PluginRegistrationConfig
        {
            Assemblies =
            [
                new PluginAssemblyConfig
                {
                    ...
                    [
                        new PluginTypeConfig
                        {
                            ...
                            Steps =
                            [
                                new PluginStepConfig { Name = "DupName", Message = "Create", Entity = "account" },
                                new PluginStepConfig { Name = "DupName", Message = "Update", Entity = "account" }
                            ]
                        }
                    ]
                }
        ...
        var ex = Assert.Throws<InvalidOperationException>(() => config.Validate());
        Assert.Contains("DupName", ex.Message);
        Assert.Contains("MyPlugins.AccountPlugin", ex.Message);
        // Actionable remediation guidance
        Assert.Contains("unique explicit Name", ex.Message);
    }

    [Fact]
    public void Validate_DuplicateAutoGeneratedStepNames_ThrowsWithActionableMessage()
    {
        // Both steps auto-generate to "MyPlugins.AccountPlugin: Create of account".
        var config = new PluginRegistrationConfig
        {
            Assemblies =
            [
                new PluginAssemblyConfig
                {
                    ...
                    [
                        new PluginTypeConfig
                        {
                            ...
                            Steps =
                            [
                                new PluginStepConfig { Message = "Create", Entity = "account" },
                                new PluginStepConfig { Message = "Create", Entity = "account" }
                            ]
                        }
                    ]
        ...
        var ex = Assert.Throws<InvalidOperationException>(() => config.Validate());
        Assert.Contains("MyPlugins.AccountPlugin: Create of account", ex.Message);
        Assert.Contains("auto-generated", ex.Message);
    }

    [Fact]
    public void Validate_ExplicitAndAutoGeneratedCollision_Throws()
    {
        // One step has explicit Name that matches the auto-generated name of another step.
        var config = new PluginRegistrationConfig
        {
            Assemblies =
            [
                    ...
                    [
                        new PluginTypeConfig
                        {
                            ...
                            [
                                new PluginStepConfig
                                {
                                    Name = "MyPlugins.AccountPlugin: Create of account",
                                    Message = "Create",
                                    Entity = "account"
                                },
                                new PluginStepConfig { Message = "Create", Entity = "account" }
    ...
    [Fact]
    public void Validate_SameNameOnDifferentPluginTypes_DoesNotThrow()
    {
        // Scope of uniqueness is per plugin type — the same name on two different types is fine.
        var config = new PluginRegistrationConfig
        {
            Assemblies =
            [
                new PluginAssemblyConfig
                {
                    ...
                    [
                        new PluginTypeConfig
                        {
                            TypeName = "MyPlugins.Plugin1",
                            Steps = [new PluginStepConfig { Name = "Shared", Message = "Create", Entity = "account" }]
                        },
                        new PluginTypeConfig
                        {
                            TypeName = "MyPlugins.Plugin2",
                            Steps = [new PluginStepConfig { Name = "Shared", Message = "Create", Entity = "account" }]
                        }
                    ]
                }
