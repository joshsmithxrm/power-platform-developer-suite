...
public sealed class LibraryReferenceGenerator : IReferenceGenerator
{
        ...
        Type type, TypeDoc typeDoc, XmlDocIndex xmlDocs, string packageName)
    {
        ...
        sb.Append("# ").Append(MdxEscape.Heading(FormatTypeName(type))).Append('\n').Append('\n');
        ...
        if (members.Count > 0)
        {
            ...
            foreach (var group in groupOrder)
            {
                ...
                {
                    sb.Append("#### ").Append(MdxEscape.Heading(m.Heading)).Append('\n').Append('\n');
    ...
    private static List<MemberRow> CollectMembers(Type type, XmlDocIndex xmlDocs, List<string> diags)
    {
        ...
        foreach (var ctor in type.GetConstructors(flags))
        {
            ...
            rows.Add(new MemberRow(
                Group: "Constructors",
                Name: ".ctor",
                Heading: $"{FormatTypeName(type)}({FormatParams(ctor.GetParameters())})",
    ...
    private static string RenderIndex(string packageShort, List<IndexEntry> entries)
    {
        ...
        foreach (var group in groupOrder)
        {
            ...
            foreach (var e in inGroup)
            {
                ...
                sb.Append("| [").Append(MdxEscape.Heading(e.Name)).Append("](").Append(e.RelativeLink).Append(") | ")
    ...
    private static string SignatureOf(ConstructorInfo ctor) =>
        $"{FormatTypeName(ctor.DeclaringType!)}({FormatParams(ctor.GetParameters())})";
    ...
    /// <summary>
    /// Formats a type's own name for use in headings. Non-generic types return
    /// <c>type.Name</c>; generic types return the C#-style name with generic
    /// parameters (e.g., <c>Processor&lt;TRequest, TResponse&gt;</c>).
    /// </summary>
